"""
Adapted from paper: Ingolfsson, T. M., Hersche, M., Wang, X., Kobayashi, N., Cavigelli, L., & Benini, L. (2020, October). 
...
class EEGTCNet(nn.Module):
    # def __init__(self, nb_classes, Chans=64, layers=16, kernel_s=10, filt=10, dropout=0.3, activation=F.relu, F1=8, D=2, kernLength=32, dropout_eeg=0.2):
    def __init__(self, nb_classes, eeg_chans=22, tcn_kernel_size=4, tcn_filters=12, dropout_tcn=0.3,
                 F1=8, D=2, eegnet_kernel_size=32, dropout_eegnet=0.2, eegnet_pooling_1=8, eegnet_pooling_2=3):
        ...
        self.tcn = TCNBlock(input_dimension=F2, tcn_kernel_size=tcn_kernel_size, tcn_filters=tcn_filters, dropout=dropout_tcn)
        self.linear = nn.Linear(tcn_filters, nb_classes)
    ...
    def forward(self, x):
        ...
        x = x[:, :, -1]
        # print('Shape of x after TCN and -1: ', x.shape)
        # x = torch.squeeze(x)
        x = self.linear(x)
        # x = self.softmax(x)
        return x
...
model = EEGTCNet(nb_classes=nclasses).to(device)

model_stats = summary(model, input_size=(batch_size, num_channels, sequence_length), verbose=0)
print(model_stats)
...
root_path_keywords = 'results_EEGTCNet_2023_someone_else_model'
...
for epoch in range(1, epochs + 1):
    ...
    # ## Calculate epoch-wise test performance
    # test_loss, test_acc, y_pred, y_test = evaluate_test(model, test_dataloader)
    # y_test = np.array(y_test).reshape(-1)
    # y_pred = np.array(y_pred).reshape(-1)
    # test_macro_f1_score.append(f1_score(y_test, y_pred, average='macro'))
    ...
    print(f'| end of epoch {epoch:3d} | time: {elapsed:5.2f}s | '
        ...
        #   f'| test macro f1 {test_macro_f1_score[-1]: 8.3f}')
    print('-' * 89)
...
# test_f1_save_path = os.path.join(results_save_folder, 'test_f1_macro_through_epochs.txt')
    
# with open(test_f1_save_path, 'w') as txtFile:
#     for value in test_macro_f1_score:
#         txtFile.write(str(value))
#         txtFile.write('\n')

# def plot_test_f1(test_macro_f1_score, results_save_folder):

#     plt.figure()
#     plt.clf()
#     plt.title("Test Macro F1 Scores")
#     plt.plot(test_macro_f1_score)
#     plt.xlabel("iterations")
#     plt.ylabel("Macro F1")
#     plt.legend()
#     plt.savefig(os.path.join(results_save_folder, 'Macro_F1_curve.png'))

# plot_test_f1(test_macro_f1_score, results_save_folder)
